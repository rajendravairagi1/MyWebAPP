<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-[var(--text)] leading-tight">Purchase #{{ $purchase->id }}</h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-[var(--card)] overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-[var(--text)]">
                    <div class="flex justify-between items-start mb-6">
                        <div>
                            <p class="text-xs text-[var(--muted)] uppercase tracking-wide">Supplier</p>
                            <p class="font-semibold text-lg">{{ $purchase->supplier->name }}</p>
                            <p class="text-sm text-[var(--muted)]">GST: {{ $purchase->supplier->gst_number ?? '—' }}</p>
                        </div>
                        <div class="text-right text-sm">
                            <p><span class="text-[var(--muted)]">Date:</span> {{ $purchase->purchase_date->format('d M Y') }}</p>
                            <p><span class="text-[var(--muted)]">Bill #:</span> {{ $purchase->bill_number ?? '—' }}</p>
                            @php
                    ...
                    <div class="flex justify-end mt-4">
                        <div class="w-64 text-sm space-y-1">
                            ...
                            <div class="flex justify-between font-semibold {{ $purchase->balanceDue() > 0 ? 'text-red-600' : 'text-[var(--muted)]' }}"><span>Balance Due</span><span>₹{{ number_format($purchase->balanceDue(), 2) }}</span></div>
                        </div>
                    </div>

                    @if ($purchase->notes)
                        <p class="text-xs text-[var(--muted)] mt-4 border-t pt-3">{{ $purchase->notes }}</p>
                    @endif
                </div>
            </div>
                    ...
                    <div class="p-6 text-[var(--text)]">
                        ...
                        <form method="POST" action="{{ route('purchases.payments.store', $purchase) }}" class="grid grid-cols-1 sm:grid-cols-4 gap-3 items-end">
                            ...
                            <div>
                                ...
                                <input type="number" step="0.01" name="amount" max="{{ $purchase->balanceDue() }}" value="{{ $purchase->balanceDue() }}" required class="w-full rounded-md border-[var(--border)] shadow-sm text-sm">
                            </div>
                            <div>
            ...
            <a href="{{ route('purchases.index') }}" class="text-sm text-[var(--muted)] hover:underline">&larr; Back to Purchases</a>
        </div>
    </div>
</x-app-layout>
