<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Nayi Purchase Entry') }}</h2>
    </x-slot>
                    ...
                    <form method="POST" action="{{ route('purchases.store') }}">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                            <div>
                                ...
                                <select name="supplier_id" required class="w-full rounded-md border-gray-300 shadow-sm text-sm">
                            ...
                            <div>
                                ...
                                <input type="text" name="bill_number" value="{{ old('bill_number') }}" class="w-full rounded-md border-gray-300 shadow-sm text-sm">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Purchase Date *</label>
                                <input type="date" name="purchase_date" value="{{ old('purchase_date', date('Y-m-d')) }}" required class="w-full rounded-md border-gray-300 shadow-sm text-sm">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Quantity *</label>
                                <input type="number" step="0.01" name="quantity" value="{{ old('quantity') }}" required class="w-full rounded-md border-gray-300 shadow-sm text-sm">
                            ...
                            <div>
                                ...
                                <input type="number" step="0.01" name="rate" value="{{ old('rate') }}" required class="w-full rounded-md border-gray-300 shadow-sm text-sm">
                        ...
                        <a href="{{ route('purchases.index') }}" class="ml-3 text-sm text-gray-500 hover:underline">Cancel</a>
                    </form>
                </div>
            </div>
        </div>
