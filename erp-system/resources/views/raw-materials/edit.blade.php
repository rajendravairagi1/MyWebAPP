<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-[var(--text)] leading-tight">{{ __('Edit Material') }} — {{ $material->name }}</h2>
    </x-slot>
            ...
            <div class="bg-[var(--card)] overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-[var(--text)]">
                    ...
                    <form method="POST" action="{{ route('raw-materials.update', $material) }}">
                        ...
                        <a href="{{ route('raw-materials.index') }}" class="ml-3 text-sm text-[var(--muted)] hover:underline">Cancel</a>
                    </form>
