<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-[var(--text)] leading-tight">{{ __('Payroll') }}</h2>
    </x-slot>
                ...
                <div class="p-6 text-[var(--text)]">
                    ...
                    <div class="flex items-center justify-between mb-6">
                        <form method="GET" action="{{ route('payroll.index') }}" class="flex items-center gap-2">
                            <select name="month" onchange="this.form.submit()" class="rounded-md border-[var(--border)] shadow-sm text-sm">
                                @foreach (range(1, 12) as $m)
                                    <option value="{{ $m }}" @selected($m == $month)>{{ \Carbon\Carbon::create(null, $m, 1)->format('F') }}</option>
                                @endforeach
                            </select>
                            <select name="year" onchange="this.form.submit()" class="rounded-md border-[var(--border)] shadow-sm text-sm">
                        ...
                        <table class="min-w-full divide-y divide-[var(--border)] text-sm">
                            <thead class="bg-[var(--bg)]">
                                <tr>
                                    ...
                                    <th class="px-4 py-2 text-right font-semibold text-[var(--muted)]">Present Days</th>
                                    <th class="px-4 py-2 text-right font-semibold text-[var(--muted)]">Gross</th>
                                    <th class="px-4 py-2 text-right font-semibold text-[var(--muted)]">Net Salary</th>
                            ...
                            <tbody class="divide-y divide-[var(--border)]">
                                @forelse ($employees as $employee)
                                    ...
                                    <tr>
                                        ...
                                        <td class="px-4 py-2">
                                            ...
                                            @else
                                                <span class="text-[var(--muted)] text-xs">Not generated</span>
                                            @endif
                                        </td>
                                ...
                                @empty
                                    <tr><td colspan="6" class="px-4 py-6 text-center text-[var(--muted)]">Koi active employee nahi hai.</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
