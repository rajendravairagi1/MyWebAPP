<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Salary Slip — {{ $slip->employee->name }} ({{ $slip->monthLabel() }})</h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8 space-y-6">
                ...
                <div class="p-6 text-gray-900">
                    <div class="flex justify-between items-start mb-6">
                        <div>
                            ...
                            <p class="text-sm text-gray-500">{{ $slip->employee->employee_code }} · {{ $slip->employee->designation }}</p>
                        </div>
                        <div class="text-right text-sm">
                            <p><span class="text-gray-500">Month:</span> {{ $slip->monthLabel() }}</p>
                    ...
                    <div class="text-sm space-y-1 border rounded-lg p-4">
                        <div class="flex justify-between"><span class="text-gray-500">Present Days</span><span>{{ $slip->present_days }}</span></div>
                        <div class="flex justify-between"><span class="text-gray-500">Per Day Salary</span><span>₹{{ number_format($slip->per_day_salary, 2) }}</span></div>
                        ...
                        @if ($slip->paid_at)
                            <div class="flex justify-between text-gray-400 text-xs pt-1"><span>Paid On</span><span>{{ $slip->paid_at->format('d M Y') }}</span></div>
                        @endif
                    </div>
                </div>
            </div>
            ...
            @if ($slip->status !== 'paid')
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 text-gray-900">
                        <h3 class="font-semibold text-gray-800 mb-4">Adjust Deductions</h3>
                        <form method="POST" action="{{ route('payroll.update', $slip) }}" class="grid grid-cols-1 sm:grid-cols-3 gap-3 items-end">
                            @csrf
                            @method('PUT')
                            <div>
                                <label class="block text-xs font-medium text-gray-600 mb-1">Advance Deduction (₹)</label>
                                <input type="number" step="0.01" name="advance_deduction" value="{{ $slip->advance_deduction }}" required class="w-full rounded-md border-gray-300 shadow-sm text-sm">
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-gray-600 mb-1">Other Deduction (₹)</label>
                                <input type="number" step="0.01" name="other_deduction" value="{{ $slip->other_deduction }}" required class="w-full rounded-md border-gray-300 shadow-sm text-sm">
                            </div>
                            <div>
            ...
            <a href="{{ route('payroll.index', ['month' => $slip->month, 'year' => $slip->year]) }}" class="text-sm text-gray-500 hover:underline">&larr; Back to Payroll</a>
        </div>
    </div>
</x-app-layout>
