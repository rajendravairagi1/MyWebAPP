<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Reports Center') }}</h2>
    </x-slot>
        ...
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div class="bg-white rounded-lg shadow-sm p-5">
                    <p class="text-xs text-gray-500 uppercase tracking-wide">Invoiced (Sales)</p>
                    <p class="text-xl font-bold mt-1">₹{{ number_format($invoiceTotal, 0) }}</p>
                </div>
                <div class="bg-white rounded-lg shadow-sm p-5">
                    <p class="text-xs text-gray-500 uppercase tracking-wide">Received (Sales)</p>
                    <p class="text-xl font-bold mt-1 text-green-600">₹{{ number_format($paymentTotal, 0) }}</p>
                </div>
                <div class="bg-white rounded-lg shadow-sm p-5">
                    <p class="text-xs text-gray-500 uppercase tracking-wide">Purchased (Raw Material)</p>
                    <p class="text-xl font-bold mt-1">₹{{ number_format($purchaseTotal, 0) }}</p>
                </div>
                <div class="bg-white rounded-lg shadow-sm p-5">
                    <p class="text-xs text-gray-500 uppercase tracking-wide">Paid (Purchase)</p>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="bg-white rounded-lg shadow-sm p-5">
                    <p class="text-xs text-gray-500 uppercase tracking-wide">Outstanding Receivables (Customers owe us)</p>
                    <p class="text-xl font-bold mt-1 text-red-600">₹{{ number_format($outstandingReceivables, 0) }}</p>
                </div>
                <div class="bg-white rounded-lg shadow-sm p-5">
                    <p class="text-xs text-gray-500 uppercase tracking-wide">Outstanding Payables (We owe suppliers)</p>
            ...
            <div class="bg-white rounded-lg shadow-sm p-6">
                <h3 class="font-semibold text-gray-800 mb-4">Job Status (selected range, by received date)</h3>
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <!-- Low Stock -->
            <div class="bg-white rounded-lg shadow-sm p-6">
                <h3 class="font-semibold text-gray-800 mb-4">Low Stock Materials</h3>
