<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-[var(--text)] leading-tight">{{ __('Edit Customer') }} — {{ $customer->name }}</h2>
    </x-slot>
            ...
            <div class="bg-[var(--card)] overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-[var(--text)]">
                    ...
                    <form method="POST" action="{{ route('customers.update', $customer) }}" enctype="multipart/form-data">
                        ...
                        <a href="{{ route('customers.index') }}" class="ml-3 text-sm text-[var(--muted)] hover:underline">Cancel</a>
                    </form>

                    @if ($customer->documents->count())
                        <div class="mt-8 border-t pt-4">
                            <h3 class="text-sm font-semibold text-[var(--muted)] mb-2">Uploaded Documents</h3>
