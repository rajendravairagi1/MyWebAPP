<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-[var(--text)] leading-tight">{{ __('Naya Job Receive Karo') }}</h2>
    </x-slot>
                    ...
                    @else
                        <form method="POST" action="{{ route('jobs.store') }}" enctype="multipart/form-data" x-data="{ rows: [{ product_id: '', quantity: 1 }] }">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                                <div>
                                    <label class="block text-sm font-medium text-[var(--muted)] mb-1">Customer *</label>
                                ...
                                <div>
                                    ...
                                    <input type="text" name="challan_number" value="{{ old('challan_number') }}" class="w-full rounded-md border-[var(--border)] shadow-sm text-sm">
                                </div>
                                <div>
                                ...
                                <div>
                                    <label class="block text-sm font-medium text-[var(--muted)] mb-1">Vehicle/Material Photo</label>
                                    <input type="file" name="photo" accept="image/*" class="w-full text-sm">
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-[var(--muted)] mb-1">Received Date *</label>
                                    <input type="date" name="received_date" value="{{ old('received_date', date('Y-m-d')) }}" required class="w-full rounded-md border-[var(--border)] shadow-sm text-sm">
                                </div>
                                <div>
                                    ...
                                    <p class="text-xs text-[var(--muted)] mt-1">1 din pehle reminder ke liye dashboard pe dikhega.</p>
                                </div>
                            ...
                            <div class="border-t pt-4 mb-6">
                                <div class="flex justify-between items-center mb-2">
                                    <h3 class="text-sm font-semibold text-[var(--muted)]">Products (Pieces)</h3>
                                ...
                                <template x-for="(row, index) in rows" :key="index">
                                    <div class="flex gap-3 mb-2 items-center">
                                        <select :name="'items['+index+'][product_id]'" x-model="row.product_id" required
                                                class="flex-1 rounded-md border-[var(--border)] shadow-sm text-sm">
                                        ...
                                        <input type="number" :name="'items['+index+'][quantity]'" x-model="row.quantity" min="1" required
                                               class="w-24 rounded-md border-[var(--border)] shadow-sm text-sm" placeholder="Qty">
                            ...
                            <a href="{{ route('jobs.index') }}" class="ml-3 text-sm text-[var(--muted)] hover:underline">Cancel</a>
                        </form>
                    @endif
                </div>
            </div>
