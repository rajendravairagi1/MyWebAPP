        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-[var(--card)] overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-[var(--text)]">
                    ...
                    <form method="POST" action="{{ route('roles.update', $role) }}">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
                            @foreach ($modules as $key => $label)
                                <label class="flex items-center gap-2 border rounded-lg px-3 py-2 {{ $role->name === 'Owner' ? 'bg-[var(--bg)] opacity-70' : 'hover:bg-[var(--bg)]' }}">
                    ...
                    <a href="{{ route('roles.index') }}" class="inline-block mt-4 text-sm text-[var(--muted)] hover:underline">&larr; Back to Roles</a>
                </div>
            </div>
        </div>
    </div>
