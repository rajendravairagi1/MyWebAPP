<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-[var(--text)] leading-tight">{{ __('Leads') }}</h2>
    </x-slot>
                ...
                <div class="p-6 text-[var(--text)]">
                    ...
                    <div class="flex justify-between items-center mb-4">
                        <div class="flex gap-1 text-sm flex-wrap">
                            <a href="{{ route('leads.index') }}" class="px-3 py-1.5 rounded-md {{ ! $status ? 'bg-[var(--brand-100)] text-[var(--brand-700)]' : 'text-[var(--muted)] hover:bg-[var(--bg)]' }}">All</a>
                            @foreach (['new' => 'New', 'contacted' => 'Contacted', 'qualified' => 'Qualified', 'converted' => 'Converted', 'lost' => 'Lost'] as $key => $label)
                                <a href="{{ route('leads.index', ['status' => $key]) }}" class="px-3 py-1.5 rounded-md {{ $status === $key ? 'bg-[var(--brand-100)] text-[var(--brand-700)]' : 'text-[var(--muted)] hover:bg-[var(--bg)]' }}">{{ $label }}</a>
                            @endforeach
                        </div>
                        ...
                        <table class="min-w-full divide-y divide-[var(--border)] text-sm">
                            <thead class="bg-[var(--bg)]">
                                <tr>
                                    ...
                                    <th class="px-4 py-2 text-left font-semibold text-[var(--muted)]">Assigned To</th>
                            ...
                            <tbody class="divide-y divide-[var(--border)]">
                                @forelse ($leads as $lead)
                                    <tr>
                                        ...
                                        <td class="px-4 py-2 text-[var(--muted)]">{{ $lead->company_name ?? '—' }}</td>
                                        <td class="px-4 py-2 text-[var(--muted)]">{{ $lead->mobile ?? '—' }}</td>
                                        <td class="px-4 py-2 text-[var(--muted)]">{{ $lead->assignedUser?->name ?? '—' }}</td>
                                        <td class="px-4 py-2">
                                ...
                                @empty
                                    <tr><td colspan="6" class="px-4 py-6 text-center text-[var(--muted)]">Koi lead nahi hai abhi.</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
