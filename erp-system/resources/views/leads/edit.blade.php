<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-[var(--text)] leading-tight">{{ __('Lead Edit') }}</h2>
    </x-slot>
            ...
            <div class="bg-[var(--card)] overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-[var(--text)]">
                    <form method="POST" action="{{ route('leads.update', $lead) }}">
                        ...
                        <a href="{{ route('leads.show', $lead) }}" class="ml-3 text-sm text-[var(--muted)] hover:underline">Cancel</a>
                    </form>
                </div>
            </div>
        </div>
