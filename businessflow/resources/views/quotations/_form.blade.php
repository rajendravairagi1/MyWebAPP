...
<div x-data="{
        customerId: '{{ old('customer_id', $quotation?->customer_id ?? $prefillCustomerId) }}',
...
<div>
    ...
    <textarea id="notes" name="notes" rows="2" class="mt-1 block w-full border-gray-300 dark:border-slate-600 dark:bg-slate-700 dark:text-gray-100 rounded-md shadow-sm focus:border-accent-500 focus:ring-accent-500">{{ old('notes', $quotation?->notes) }}</textarea>
</div>
<div>
    <x-input-label for="terms" :value="__('Terms')" />
    <textarea id="terms" name="terms" rows="2" class="mt-1 block w-full border-gray-300 dark:border-slate-600 dark:bg-slate-700 dark:text-gray-100 rounded-md shadow-sm focus:border-accent-500 focus:ring-accent-500">{{ old('terms', $quotation?->terms) }}</textarea>
</div>
