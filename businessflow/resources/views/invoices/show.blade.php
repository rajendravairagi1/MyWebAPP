    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="flex flex-wrap gap-3">
                <a href="{{ route('invoices.pdf', $invoice) }}" class="px-4 py-2 border border-gray-300 dark:border-slate-600 text-gray-700 dark:text-gray-300 text-sm font-medium rounded-md hover:bg-gray-50 dark:hover:bg-slate-700">{{ __('Download PDF') }}</a>
                ...
                <script>document.addEventListener('DOMContentLoaded', function () { preloadPdf('{{ route('invoices.pdf', $invoice) }}'); });</script>
                ...
                @if ($invoice->status === 'draft')
                    <form method="POST" action="{{ route('invoices.mark-sent', $invoice) }}">
                        @csrf
                        <button class="px-4 py-2 border border-gray-300 dark:border-slate-600 text-gray-700 dark:text-gray-300 text-sm font-medium rounded-md hover:bg-gray-50 dark:hover:bg-slate-700">{{ __('Mark as Sent') }}</button>
                    </form>
                @endif
            </div>
            ...
            <div class="bg-white dark:bg-slate-800 shadow-sm rounded-lg overflow-hidden">
                ...
                <div class="px-5 py-4 flex justify-end">
                    <table class="text-sm w-64">
                        ...
                        <tr><td class="py-1 text-gray-500 dark:text-gray-400">{{ __('GST') }}</td><td class="py-1 text-right">{{ number_format($invoice->tax_total, 2) }}</td></tr>
                        @if ($invoice->tax_total > 0 && ($__gstNumber = \App\Models\Business::find(\App\Support\Tenant::id())?->gst_number))
            ...
            <div class="bg-white dark:bg-slate-800 shadow-sm rounded-lg overflow-hidden">
                ...
                @if ($invoice->balanceDue() > 0)
                    <form method="POST" action="{{ route('payments.store', $invoice) }}" x-data="{ method: 'cash', paymentAccountId: '' }" class="p-5 border-t border-gray-100 dark:border-slate-700 grid grid-cols-2 sm:grid-cols-5 gap-3 items-end">
                        @csrf
                        ...
                        <div>
                            ...
                            <select id="method" name="method" x-model="method" x-on:change="paymentAccountId = ''" class="mt-1 block w-full text-sm border-gray-300 dark:border-slate-600 dark:bg-slate-700 dark:text-gray-100 rounded-md">
                        ...
                        @if ($paymentAccounts->isNotEmpty())
                            <div class="col-span-2 sm:col-span-5">
                                <x-input-label for="payment_account_id" x-text="method === 'cash' ? '{{ __('Who is holding this cash? (optional)') }}' : '{{ __('Received In (optional)') }}'" class="text-xs" />
                                <select id="payment_account_id" name="payment_account_id" x-model="paymentAccountId" class="mt-1 block w-full text-sm border-gray-300 dark:border-slate-600 dark:bg-slate-700 dark:text-gray-100 rounded-md">
                                    <option value="">{{ __('— Not specified —') }}</option>
                                    @foreach ($paymentAccounts as $account)
