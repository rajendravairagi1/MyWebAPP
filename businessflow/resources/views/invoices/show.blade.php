<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Invoice') }} {{ $invoice->number }}</h2>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="flex flex-wrap gap-3">
                <a href="{{ route('invoices.pdf', $invoice) }}" class="px-4 py-2 border border-gray-300 text-gray-700 text-sm font-medium rounded-md hover:bg-gray-50">{{ __('Download PDF') }}</a>
            ...
            <div class="bg-white shadow-sm rounded-lg p-6 grid grid-cols-1 sm:grid-cols-4 gap-4 text-sm">
                <div>
                ...
                <div>
                    ...
                    <div class="text-gray-900 font-medium">{{ number_format($invoice->total, 2) }}</div>
                </div>
                <div>
                    <div class="text-gray-500">{{ __('Balance due') }}</div>
            ...
            <div class="bg-white shadow-sm rounded-lg overflow-hidden">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @foreach ($invoice->items as $item)
                            <tr>
                                ...
                                <td class="px-5 py-3 text-right text-gray-600">{{ rtrim(rtrim($item->quantity, '0'), '.') ?: '0' }}</td>
                ...
                <div class="px-5 py-4 flex justify-end">
                    <table class="text-sm w-64">
                        <tr><td class="py-1 text-gray-500">{{ __('Subtotal') }}</td><td class="py-1 text-right">{{ number_format($invoice->subtotal, 2) }}</td></tr>
                        <tr><td class="py-1 text-gray-500">{{ __('Discount') }}</td><td class="py-1 text-right">{{ number_format($invoice->discount_total, 2) }}</td></tr>
                        <tr><td class="py-1 text-gray-500">{{ __('Tax') }}</td><td class="py-1 text-right">{{ number_format($invoice->tax_total, 2) }}</td></tr>
            ...
            <div class="bg-white shadow-sm rounded-lg overflow-hidden">
                <div class="px-5 py-3 border-b border-gray-100 font-medium text-gray-800">{{ __('Payments') }}</div>
                        ...
                        @foreach ($invoice->payments as $payment)
                            <li class="px-5 py-3 flex items-center justify-between">
                                <div>
                                    ...
                                    <span class="text-gray-500">· {{ $payment->paid_at->format('d M Y') }}</span>
                                    @if ($payment->method)<span class="text-gray-500">· {{ ucfirst(str_replace('_', ' ', $payment->method)) }}</span>@endif
                ...
                @if ($invoice->balanceDue() > 0)
                    <form method="POST" action="{{ route('payments.store', $invoice) }}" class="p-5 border-t border-gray-100 grid grid-cols-2 sm:grid-cols-5 gap-3 items-end">
                        @csrf
