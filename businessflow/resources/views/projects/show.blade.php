    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                ...
                <div class="text-xs text-gray-500 capitalize">{{ str_replace('_', ' ', $project->type) }} @if($project->location) · {{ $project->location }} @endif</div>
            </div>
        ...
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <div class="bg-white shadow-sm rounded-lg p-5">
                    ...
                    <div class="mt-1 text-2xl font-semibold text-gray-900">{{ number_format($cost, 0) }}</div>
                </div>
                <div class="bg-white shadow-sm rounded-lg p-5">
                    <div class="text-xs uppercase tracking-wide text-gray-500">{{ __('Booked value') }}</div>
                    <div class="mt-1 text-2xl font-semibold text-gray-900">{{ number_format($invoiced, 0) }}</div>
                </div>
                <div class="bg-white shadow-sm rounded-lg p-5">
                    <div class="text-xs uppercase tracking-wide text-gray-500">{{ __('Received') }}</div>
                    <div class="mt-1 text-2xl font-semibold text-gray-900">{{ number_format($revenue, 0) }}</div>
                </div>
                <div class="bg-white shadow-sm rounded-lg p-5">
            ...
            @if ($costsByCategory->isNotEmpty())
                <div class="bg-white shadow-sm rounded-lg p-5">
                    <div class="text-xs uppercase tracking-wide text-gray-500 mb-3">{{ __('Cost by category') }}</div>
                    <div class="flex flex-wrap gap-4 text-sm">
                        @foreach ($costsByCategory as $category => $amount)
                            <div class="px-3 py-2 bg-gray-50 rounded-md">
                                <span class="capitalize text-gray-600">{{ $category }}:</span>
                                <span class="font-medium text-gray-900">{{ number_format($amount, 0) }}</span>
                            </div>
                        @endforeach
                    </div>
                </div>
            ...
            <div class="bg-white shadow-sm rounded-lg overflow-hidden">
                <div class="px-5 py-3 border-b border-gray-100 font-medium text-gray-800">{{ __('Units') }} ({{ $project->units->count() }})</div>
                    ...
                    <table class="min-w-full text-sm">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @foreach ($project->units as $unit)
                                <tr>
                                    <td class="px-5 py-2 font-medium text-gray-900">{{ $unit->unit_number }}</td>
                                    <td class="px-5 py-2 text-gray-600">{{ $unit->type }}</td>
                                    <td class="px-5 py-2 text-right text-gray-600">{{ $unit->area_sqft ?? '—' }}</td>
                                    <td class="px-5 py-2 text-right text-gray-600">{{ number_format($unit->price, 0) }}</td>
                ...
                <form method="POST" action="{{ route('project-units.store', $project) }}" class="p-5 border-t border-gray-100 grid grid-cols-2 sm:grid-cols-6 gap-3 items-end">
                    @csrf
                    <div>
            ...
            {{-- Costs --}}
            <div class="bg-white shadow-sm rounded-lg overflow-hidden">
                <div class="px-5 py-3 border-b border-gray-100 font-medium text-gray-800">{{ __('Cost entries') }} ({{ $project->costs->count() }})</div>
                    ...
                    <table class="min-w-full text-sm">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @foreach ($project->costs as $entry)
                                <tr>
                                    <td class="px-5 py-2 text-gray-500">{{ $entry->spent_on->format('d M Y') }}</td>
                                    <td class="px-5 py-2 text-gray-600 capitalize">{{ $entry->category }}</td>
                                    <td class="px-5 py-2 text-gray-900">{{ $entry->description }}</td>
                                    <td class="px-5 py-2 text-gray-600">{{ $entry->vendor }}</td>
                                    <td class="px-5 py-2 text-right text-gray-900">{{ number_format($entry->amount, 2) }}</td>
                                    <td class="px-5 py-2 text-right">
                ...
                <form method="POST" action="{{ route('project-costs.store', $project) }}" class="p-5 border-t border-gray-100 grid grid-cols-2 sm:grid-cols-6 gap-3 items-end">
                    @csrf
                    <div>
                        ...
                        <ul class="divide-y divide-gray-100 text-sm">
                            @foreach ($project->quotations as $quotation)
                                <li class="px-5 py-3 flex items-center justify-between">
                                    ...
                                    <div class="flex items-center gap-3">
                                        <span class="text-gray-600">{{ number_format($quotation->total, 0) }}</span>
                        ...
                        <ul class="divide-y divide-gray-100 text-sm">
                            @foreach ($project->invoices as $invoice)
                                <li class="px-5 py-3 flex items-center justify-between">
                                    ...
                                    <div class="flex items-center gap-3">
                                        <span class="text-gray-600">{{ number_format($invoice->total, 0) }}</span>
