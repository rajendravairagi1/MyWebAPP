...
<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-2 min-w-0">
            <a href="{{ route('investors.index') }}" class="shrink-0 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200" title="{{ __('Back') }}">
            ...
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-100 leading-tight truncate">{{ $investor->name }}</h2>
        </div>
    </x-slot>
    ...
    <x-modal name="edit-investor" max-width="md">
        <form method="POST" action="{{ route('investors.update', $investor) }}" class="p-6 space-y-4">
            ...
            <div>
                ...
                <input type="tel" id="edit_phone" name="phone" value="{{ $investor->phone }}"
                    class="mt-1 block w-full border-gray-300 dark:border-slate-600 dark:bg-slate-700 dark:text-gray-100 rounded-md shadow-sm focus:border-accent-500 focus:ring-accent-500">
            </div>
            <div>
