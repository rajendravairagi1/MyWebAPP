<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-100 leading-tight">{{ __('Schedule Follow-up') }}</h2>
    </x-slot>
            ...
            <div class="bg-white dark:bg-slate-800 shadow-sm rounded-lg p-6">
                <form method="POST" action="{{ route('followups.store') }}" x-data="{ category: '{{ old('category', 'general') }}' }" class="space-y-6">
                    ...
                    <div class="flex justify-end gap-3">
                        <a href="{{ route('followups.index') }}" class="px-4 py-2 text-sm text-gray-600 dark:text-gray-400">{{ __('Cancel') }}</a>
