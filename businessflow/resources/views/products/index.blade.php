    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-4">
            ...
            <div class="bg-white shadow-sm rounded-lg overflow-hidden">
                @if ($products->isEmpty())
                    <div class="p-6 text-sm text-gray-500">{{ __('No products or services yet.') }}</div>
                @else
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @foreach ($products as $product)
                                <tr>
                                    ...
                                    <td class="px-5 py-3 text-gray-600 capitalize">{{ $product->type }}</td>
                                    <td class="px-5 py-3 text-gray-600">{{ $product->sku }}</td>
                                    <td class="px-5 py-3 text-right text-gray-600">{{ number_format($product->price, 2) }}</td>
                                    <td class="px-5 py-3 text-right">
                                        @if ($product->stock_qty !== null)
                                            <span class="{{ $product->isLowStock() ? 'text-red-600 font-medium' : 'text-gray-600' }}">{{ $product->stock_qty }}</span>
                                        @else
