<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Edit Product / Service') }}</h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <div class="bg-white shadow-sm rounded-lg p-6">
                <form method="POST" action="{{ route('products.update', $product) }}" class="space-y-6">
                    ...
                    <div class="flex justify-end gap-3">
                        <a href="{{ route('products.index') }}" class="px-4 py-2 text-sm text-gray-600">{{ __('Cancel') }}</a>
            ...
            <div class="bg-white shadow-sm rounded-lg p-6 flex items-center justify-between">
                <div class="text-sm text-gray-600">{{ __('Deleting cannot be undone.') }}</div>
