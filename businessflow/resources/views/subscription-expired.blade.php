<x-guest-layout>
    <div class="text-center space-y-4">
        ...
        <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100">{{ $paused ? __('Account paused') : __('Subscription expired') }}</h2>

        <p class="text-sm text-gray-600 dark:text-gray-400">
            @if ($paused)
                {{ __('Access to') }} <span class="font-medium text-gray-800 dark:text-gray-100">{{ $business?->name ?? __('this account') }}</span> {{ __('has been paused by the admin.') }}
            @elseif ($business)
