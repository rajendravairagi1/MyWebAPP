        ...
        <div x-data="{
            ...
            <aside
                ...
                @php
                    ...
                    $signupRequestsBadge = $pendingSignupRequestsCount > 0
                        ? '<span class="inline-flex items-center justify-center h-4 min-w-[1rem] px-1 rounded-full bg-red-600 text-white text-[10px] font-semibold">'.$pendingSignupRequestsCount.'</span>'
                        : null;
                    ...
                    $items = [
                        ...
                        'admin-users' => ['label' => __('Admin'), 'href' => route('admin.users.index'), 'pattern' => 'admin.users.*', 'icon' => '<path stroke-linecap="round" stroke-linejoin="round" d="M18 18.72a9.094 9.094 0 003.741-.479 3 3 0 00-4.682-2.72m.94 3.198l.001.031c0 .225-.012.447-.037.666A11.944 11.944 0 0112 21c-2.17 0-4.207-.576-5.963-1.584A6.062 6.062 0 016 18.719m12 0a5.971 5.971 0 00-.941-3.197m0 0A5.995 5.995 0 0012 12.75a5.995 5.995 0 00-5.058 2.772m0 0a3 3 0 00-4.681 2.72 8.986 8.986 0 003.74.477m.94-3.197a5.971 5.971 0 00-.94 3.197M15 6.75a3 3 0 11-6 0 3 3 0 016 0zm6 3a2.25 2.25 0 11-4.5 0 2.25 2.25 0 014.5 0zm-13.5 0a2.25 2.25 0 11-4.5 0 2.25 2.25 0 014.5 0z" />', 'visible' => $isPlatformAdmin],
                        'admin-backups' => ['label' => __('Backups'), 'href' => route('admin.backups.index'), 'pattern' => 'admin.backups.*', 'icon' => '<path stroke-linecap="round" stroke-linejoin="round" d="M20.25 7.5l-.625 10.632a2.25 2.25 0 01-2.247 2.118H6.622a2.25 2.25 0 01-2.247-2.118L3.75 7.5M10 11.25h4M3.375 7.5h17.25c.621 0 1.125-.504 1.125-1.125v-1.5c0-.621-.504-1.125-1.125-1.125H3.375c-.621 0-1.125.504-1.125 1.125v1.5c0 .621.504 1.125 1.125 1.125z" />', 'visible' => $isPlatformAdmin],
                        'admin-signup-requests' => ['label' => __('Signup Requests'), 'href' => route('admin.signup-requests.index'), 'pattern' => 'admin.signup-requests.*', 'icon' => '<path stroke-linecap="round" stroke-linejoin="round" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />', 'visible' => $isPlatformAdmin, 'badge' => $signupRequestsBadge],
            ...
            <div class="flex-1 flex flex-col min-w-0 min-h-screen">
                ...
                <header class="sticky top-0 z-20 min-h-16 shrink-0 bg-white dark:bg-slate-800 border-b border-gray-200 dark:border-slate-700 flex items-center gap-4 px-4 sm:px-6 py-2">
                    ...
                    <x-dropdown align="right" width="w-80">
                        <x-slot name="trigger">
                            <button type="button" class="relative text-gray-500 dark:text-slate-400 hover:text-gray-700 dark:hover:text-slate-200">
                                ...
                                {{-- The plan-expiry notice below is deliberately excluded from this
                        ...
                        <x-slot name="content">
                            ...
                            @if ($subscriptionDaysRemaining !== null)
                                <div class="px-4 py-3 bg-amber-50 dark:bg-amber-900/20 border-b border-amber-200 dark:border-amber-800">
                                    ...
                                    <div class="text-xs text-amber-700 dark:text-amber-500 mt-0.5">{{ __('Valid till') }} {{ $subscriptionExpiresOn->format('d M Y') }} — {{ __('renew to avoid losing access.') }}</div>
                                    <a href="{{ route('billing.show') }}" class="mt-2 inline-flex items-center justify-center w-full px-3 py-1.5 bg-amber-600 text-white text-xs font-semibold rounded-md hover:bg-amber-700">
                                        {{ __('Pay Now') }}
                                    </a>
                                </div>
                            @endif
