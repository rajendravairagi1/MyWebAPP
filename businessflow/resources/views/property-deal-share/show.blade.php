...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <meta name="description" content="{{ $deal->address }}">
    ...
    <body class="font-sans antialiased bg-gray-50 dark:bg-slate-900 text-gray-900 dark:text-gray-100">
        <div class="max-w-2xl mx-auto px-4 py-6 space-y-5">
            <div class="flex items-center gap-3">
                @if ($deal->business?->logoDataUri())
                    <img src="{{ $deal->business->logoDataUri() }}" alt="{{ $deal->business->name }}" class="h-10">
                @endif
                <div class="min-w-0">
                    <div class="font-semibold tracking-tight truncate">{{ $deal->business?->name ?? config('app.name') }}</div>
                    @php $contactLine = $deal->business ? collect([$deal->business->phone, $deal->business->email, $deal->business->website])->filter()->implode(' · ') : ''; @endphp
                    @if ($contactLine)
            ...
            @if ($photos->isNotEmpty())
                <div x-data="{ active: 0, count: {{ $photos->count() }} }" class="relative rounded-xl overflow-hidden bg-black">
                    @foreach ($photos as $i => $photo)
                        <img x-show="active === {{ $i }}" x-cloak src="{{ route('property-deal-share.photo', [$deal->share_token, $photo]) }}" alt="{{ $deal->property_title }}" class="w-full h-72 object-cover">
                    @endforeach
                    @if ($photos->count() > 1)
            ...
            <div class="bg-white dark:bg-slate-800 shadow-sm rounded-xl p-5 space-y-3">
                <div class="flex items-start justify-between gap-3">
                    <div>
                        <h1 class="text-lg font-semibold">{{ $deal->property_title }}</h1>
                        @if ($deal->address)
                            <div class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">{{ $deal->address }}</div>
                        @endif
                    </div>
                </div>

                @if ($deal->asking_price)
                    <div class="text-2xl font-bold text-accent-600">{{ $deal->business?->currencySymbol() ?? '₹' }}{{ number_format($deal->asking_price, 0) }}</div>
                @endif
            </div>
                    ...
                    <div class="flex items-center justify-between gap-3">
                        <div class="min-w-0">
                            @if ($deal->contact_name)
                                <div class="font-medium truncate">{{ $deal->contact_name }}</div>
                            @endif
                            @if ($deal->contact_email)
                                <div class="text-sm text-gray-500 dark:text-gray-400 truncate">{{ $deal->contact_email }}</div>
                            @endif
                        </div>
                        @if ($deal->contact_phone)
                            <a href="tel:{{ $deal->contact_phone }}" class="shrink-0 inline-flex items-center gap-1.5 h-10 px-4 rounded-lg text-sm font-semibold whitespace-nowrap bg-accent-600 text-white hover:bg-accent-700">
