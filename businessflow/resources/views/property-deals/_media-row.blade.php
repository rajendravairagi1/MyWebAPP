<li class="py-3 flex items-center justify-between gap-4">
    <a href="{{ route('property-deal-media.show', [$deal, $item]) }}" target="_blank" rel="noopener" class="flex items-center gap-2 min-w-0 text-accent-600 hover:underline">
        @if ($item->isImage())
            <img src="{{ route('property-deal-media.show', [$deal, $item]) }}" alt="{{ $item->original_name }}" loading="lazy" class="h-8 w-8 object-cover rounded shrink-0 border border-gray-200 dark:border-slate-700">
        @else
    ...
    <div class="flex items-center gap-3 shrink-0 text-xs text-gray-400">
        ...
        <form method="POST" action="{{ route('property-deal-media.destroy', [$deal, $item]) }}" onsubmit="return confirm('{{ __('Delete this file?') }}')">
            @csrf
            @method('DELETE')
